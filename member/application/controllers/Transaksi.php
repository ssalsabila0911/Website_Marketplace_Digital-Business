...
class Transaksi extends CI_Controller {
	...
	function index(){
		...
		//id_member yang login
		$id_member = $this->session->userdata("id_member");


		//model Mtransaksi dan fungsi transaksi_member_beli (id_member yang login)
	...
	function detail($id_transaksi) {
		...
		if ($data['transaksi']['status_transaksi']=="pesan") {
			...
			//cek midtrans apakah transaksi sudah masuk/sudah dibayar atau belum
			$curl = curl_init();
			curl_setopt_array($curl, array(
			  ...
			  CURLOPT_HTTPHEADER => array(
			    "accept: application/json","authorization: Basic U0ItTWlkLXNlcnZlci04WkowVFZZcm1nYlVIWWhXMTlsYmtOdFI6YW1pa29t"
			  ),
			));
